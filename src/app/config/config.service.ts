...
export class ConfigService {
  ...
  invalidURL: Subject<boolean> = new Subject<boolean>();
  ...
  exportAll() {
    ...
    return this.http
      .get(environment.apiBaseUrl + '/fhir/Patient/$export', {
        headers: headers,
        observe: 'response'
      })
      .pipe(
  ...
  private handleError(operation, result) {
    return (error: any) => {
      ...
      if (operation === 'saveConfig') {
        ...
        operation === 'exportAll' ||
        operation === 'getByGroupDateandResources' ||
        operation === 'getGroupById' ||
        operation === 'getByDateandResources' ||
        operation === 'getBulkDataByContentLocation' ||
        operation === 'getByGroupDateandResources'
      ) {
        this.isLoadingResults = false;
        this.loading.next(this.isLoadingResults);
